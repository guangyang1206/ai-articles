...
# 配置SSH密钥（从流水线变量注入）
mkdir -p ***REMOVED***.ssh
echo "${deploy_ssh_key}" > ***REMOVED***
chmod 600 ***REMOVED***
ssh-keyscan -H ***REMOVED*** >> ***REMOVED***.ssh/known_hosts 2>/dev/null
...
rsync -avz --delete \
  ...
  -e "ssh -i ***REMOVED*** -o StrictHostKeyChecking=no" \
  ./ ubuntu@***REMOVED***:/var/www/my-space/

echo "✅ 文件同步完成"

# 服务器上设置权限并重载nginx
ssh -i ***REMOVED*** -o StrictHostKeyChecking=no ubuntu@***REMOVED*** << 'REMOTE'
...
# 健康检查
sleep 2
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 http://***REMOVED***)
...
```bash
echo "测试网络连通性..."
timeout 5 bash -c 'cat < /dev/null > /dev/tcp/***REMOVED***/22' && echo "✅ 端口可达" || echo "❌ 端口不可达"
```
...
# 配置SSH密钥（Base64解码）
mkdir -p ***REMOVED***.ssh
echo "${deploy_ssh_key}" | base64 -d > ***REMOVED***
chmod 600 ***REMOVED***

# 验证密钥格式
echo "密钥文件前30字符: $(head -c 30 ***REMOVED***)"
if head -1 ***REMOVED*** | grep -q "BEGIN"; then
    echo "✅ SSH密钥格式正确"
else
...
# 测试SSH连接
echo "测试SSH连接..."
ssh -i ***REMOVED*** -o StrictHostKeyChecking=no -o ConnectTimeout=10 ubuntu@***REMOVED*** "echo '✅ SSH连接成功'" || {
    echo "❌ SSH连接失败"
    echo "密钥文件信息:"
    ls -la ***REMOVED***
    file ***REMOVED***
...
❌ SSH连接失败
密钥文件信息:
-rw------- 1 root root 1678 5月   2 22:09 ***REMOVED***
***REMOVED***: PEM RSA private key
```
...
kex_exchange_identification: Connection closed by remote host
密钥文件信息:
-rw------- 1 root root 1678 5月   2 22:18 ***REMOVED***
***REMOVED***: PEM RSA private key
```
